...
- 실습 : (19_array_object.html)
  ...
  ⑥ unshift() :
      ...
      <select name="서버로 전송될 이름" id="고유_아이디">
        <option value="서버로 전송될 값">사용자에게 보여줄 값</option>
      <select>
  ...
  ⑨ slice(start, end) : end - 1까지 잘라내 배열로 반환

  ⑩ sort() : 배열의 요소를 정렬해주는 함수로 매개변수가 함수이다. => 문자열 기준

  // 따라서 숫자일 경우 별도의 알고리즘을 동작시켜야 한다.
  // 자바스크립트의 비교 함수의 동작 원리 =>
  엔진이 내부적으로 알고리즘(크롬의 경우 TimSort 알고리즘)을 갖고 있으며
  함수를 매개변수로 사용하며, 함수는 파라미터로 a, b를 갖는다.
  반환값에 따라 다음과 같이 동작한다.
  if (a > b) return 1; // 양수이면 자리를 바꾼다.
  else if (a < b) return -1; // 음수이면 자리를 바꾸지 않는다.
  else 0; // 순서를 바꾸지 않는다.
  => if, else if, else의 세 줄을 한 줄로 바꾸면...

  ```
  const 결과배열 = 대상배열.sort((a,b) => {
    return a - b; (오름차순의 경우)
    return b - a; (내림차순의 경우)
  })
  ```
  ...
  // 실무 활용 예제: 사용자가 카테고리를 선택하고, 특정 조건의 상품만 보겠다고 체크 했을 때, 프론트엔드에서 데이터를 거르는 방법

  ⑫ map() : 배열의 모든 요소를 하나씩 돌면서 실행하고 새로운 배열을 생성하는 함수

  // 실무 활용 예제: 서버에서 받아온 json 문자열을 화면에 보여줄 html 태그나 ui 컴포넌트 배열로 변환할 때, for ~ push() 보다 편의성이 좋다.

  ⑬ reduce(p1, p2, p3, p4) : 배열 요소의 누적 합을 구하는 함수, 또는 특정한 조건에 해당하는 값만 추출하는 함수 // p1: accumulator => 누적 값 // p2: currentValue => 현재 값
